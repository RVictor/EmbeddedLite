...
public:
	 ...
	 * @param path ctags
	 */
	void SetEmbeddedLiteIndexerPath(const wxString& path);
	 ...
	 *        string will be parsed by EmbeddedLite to determine the current scope
	 ...
	 * @param word the token under the cursor
	 * @param text scope where token was found
	 * @param gotoImpl set to true, if you wish that EmbeddedLite will find the implementation, false to declaration
	...
	/**
	 * @brief get the scope name. EmbeddedLite assumes that the caret is placed at the end of the 'scope'
