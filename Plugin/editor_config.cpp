...
bool EditorConfig::DoLoadDefaultSettings(const wxString& strConfigDir)
{
	//try to load the default settings
	m_fileName = wxFileName(strConfigDir + wxT("/embeddedlite.xml.default"));
	...
	if ( !m_fileName.FileExists() ) {
		...
		wxString content;
		content << wxT("<EmbeddedLite Revision=\"")
		<< m_svnRevision
		<< wxT("\"")
		<< wxT(" Version=\"")
		<< m_version
...
bool EditorConfig::Load(const wxString& strConfigDir)
{
	//first try to load the user's settings
	m_fileName = wxFileName(strConfigDir + wxT("/embeddedlite.xml"));
	m_fileName.MakeAbsolute();
	bool userSettingsLoaded(false);
	bool loadSuccess       (false);

	if (!m_fileName.FileExists()) {
		loadSuccess = DoLoadDefaultSettings(strConfigDir);
...
/*rvv
	...
	if ( userSettingsLoaded ) {
		if(!found || (found && version != this->m_version)) {
			if(DoLoadDefaultSettings(strConfigDir) == false) {
				return false;
			}
		}
	}
*/
	...
	m_fileName.SetFullName(wxT("embeddedlite.xml"));
	return true;
}
