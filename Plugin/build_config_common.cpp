...
: m_confType(confType)
{
	if (node) {
		...
		// read the assembler options
		wxXmlNode *pAsm = XmlUtils::FindFirstByTagName(node, wxT("Assembler"));
		if (NULL != pAsm) {
			m_assemblerOptions = XmlUtils::ReadString(pAsm, wxT("Options"));
			wxXmlNode *child = pAsm->GetChildren();
			while (NULL != child) {
				if (child->GetName() == wxT("IncludePath")) {
					m_AsmIncludePath.Add(XmlUtils::ReadString(child, wxT("Value")));
				}
				child = child->GetNext();
			}
		}
...
wxXmlNode *BuildConfigCommon::ToXml() const
{
	...
	// create the assembler node
	wxXmlNode *pAsm = new wxXmlNode(NULL, wxXML_ELEMENT_NODE, wxT("Assembler"));
	pAsm->AddProperty(wxT("Options"), m_assemblerOptions);
	node->AddChild(pAsm);

	for (i=0; i<m_AsmIncludePath.GetCount(); i++) {
		wxXmlNode *option = new wxXmlNode(NULL, wxXML_ELEMENT_NODE, wxT("IncludePath"));
		option->AddProperty(wxT("Value"), m_AsmIncludePath.Item(i));
		pAsm->AddChild(option);
	}
...
void BuildConfigCommon::SetAsmIncludePath(const wxString &path)
{
	FillFromSmiColonString(m_AsmIncludePath, path);
}
...
wxString BuildConfigCommon::GetAsmIncludePath() const
{
	return ArrayToSmiColonString(m_AsmIncludePath);
}
