...
void MacrosDlg::Initialize()
{
	...
	case MacrosProject:
		...
		AddMacro(wxT("$(EmbeddedLitePath)"),          wxT("Expands to EmbeddedLite's startup directory on (e.g. on Unix it exapnds to ~/.embeddedlite/"));
	...
	case MacrosCompiler:
		...
		AddMacro(wxT("$(AssemblerName)"), wxT("Expands to the assembler name as set in the Tools tab"));
		...
		AddMacro(wxT("$(AsmOptions)"), wxT("Expands to the assembler options as set in the project settings"));
