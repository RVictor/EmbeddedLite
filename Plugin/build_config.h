...
private:
	...
	wxString m_buildAsmWithGlobalSettings;
...
public:
	...
	bool IsAssemblerRequired() const {
		return m_assemblerRequired;
	}
	...
	void SetAsmIncludePath(const wxArrayString &paths) {
		m_commonConfig.SetAsmIncludePath(paths);
	}
  ...
  void SetLibraries(const wxString &libs);
	...
	void SetAssemblerRequired(bool required) {
		m_assemblerRequired = required;
	}
  ...
  void SetBuildCmpWithGlobalSettings(const wxString &buildType) {
	...
	const wxString& GetBuildAsmWithGlobalSettings() const {
		return m_buildAsmWithGlobalSettings;
	}
  void SetBuildAsmWithGlobalSettings(const wxString &buildType) {
		m_buildAsmWithGlobalSettings = buildType;
	}
