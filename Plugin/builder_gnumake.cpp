...
bool
BuilderGnuMake::Export(const wxString &project, const wxString &confToBuild, bool isProjectOnly, bool force, wxString &errMsg)
{
	TIMER_START();
	...
	ProjectPtr proj = SolutionST::Get()->FindProjectByName(project, errMsg);
	if (NULL == proj) 
	{
	...
	{
		BuildConfigPtr bldConf = SolutionST::Get()->GetProjBuildConf(project, confToBuild);
		if (!bldConf)
		{
	...
	if (!isProjectOnly)
	{
		...
		{
			ProjectPtr dependProj = SolutionST::Get()->FindProjectByName(depsArr.Item(i), errMsg);
	...
	buildTool = SolutionST::Get()->ExpandVariables(buildTool);
	...
	if (!isProjectOnly)
	{
		for (size_t i=0; i<depsArr.GetCount(); i++)
		{
			...
			BuildConfigPtr dependProjbldConf = SolutionST::Get()->GetProjBuildConf(dependProj->GetName(), projectSelConf);
	...
	if (isProjectOnly && !confToBuild.IsEmpty())
	{
...
void
BuilderGnuMake::GenerateMakefile(ProjectPtr proj, const wxString &confToBuild, bool force)
{
	wxString pname (proj->GetName());
	...
	BuildConfigPtr bldConf = SolutionST::Get()->GetProjBuildConf(proj->GetName(), confToBuild);
	if (NULL == bldConf)
	{
		return;
	}
	...
	//in case the project is type exe or dll, force link
...
void
BuilderGnuMake::CreateMakeDirsTarget(BuildConfigPtr bldConf, const wxString &targetName, wxString &text)
{
	text << wxT("\n");
	text << targetName << wxT(":\n");
...
void
BuilderGnuMake::CreateSrcList(ProjectPtr proj, const wxString &confToBuild, wxString &text)
{
	...
	for (size_t i=0; i<files.size(); i++)
	{
		...
		if ( (ft.kind == Compiler::CmpFileKindResource) && (NULL != bldConf) && !bldConf->IsResCompilerRequired() )
		{
...
void
BuilderGnuMake::CreateObjectList(ProjectPtr proj, const wxString &confToBuild, wxString &text)
{
	std::vector<wxFileName> files;
	proj->GetFiles(files);
	text << wxT("Objects=");
...
void
BuilderGnuMake::CreateFileTargets(ProjectPtr proj, const wxString &confToBuild, wxString &text)
{
	...
	bool supportPreprocessOnlyFiles = !cmp->GetSwitch(wxT("PreprocessOnly")).IsEmpty() && !cmp->GetPreprocessSuffix().IsEmpty();
...
void
BuilderGnuMake::CreateCleanTargets(ProjectPtr proj, const wxString &confToBuild, wxString &text)
{
...
//rvv		text << wxT("ProjectPath            := \"") << proj->GetFileName().GetPath(wxPATH_GET_VOLUME, wxPATH_UNIX) << wxT("\"\n");
	...
	text << wxT("EmbeddedLitePath           :=\"") << SolutionST::Get()->GetStartupDir() << wxT("\"\n");
...
wxString BuilderGnuMake::ParseIncludePath(const wxString &paths, const wxString &projectName, const wxString &selConf)
{
	...
	while (tkz.HasMoreTokens()) {
		...
		TrimString(path);
		path = ExpandAllVariables(path, SolutionST::Get(), projectName, selConf, wxEmptyString);
...
wxString BuilderGnuMake::GetBuildCommand(const wxString &project, const wxString &confToBuild)
{
	...
	cmd << buildTool << wxT(" ") << FixPath(SolutionST::Get()->GetName() + wxT("_sol.mk"));    //rvv
	return cmd;
}
...
wxString BuilderGnuMake::GetCleanCommand(const wxString &project, const wxString &confToBuild)
{
	...
	BuildMatrixPtr matrix = SolutionST::Get()->GetBuildMatrix();
	wxString type = Builder::NormalizeConfigName(matrix->GetSelectedConfigurationName());
	cmd << buildTool << wxT(" ") << FixPath(SolutionST::Get()->GetName() + wxT("_sol.mk")) << wxT(" clean");   //rvv
	return cmd;
}
