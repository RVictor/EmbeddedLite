...
private:
	bool DoSave() const;
	bool DoLoadDefaultSettings(const wxString& strConfigDir);

public:

	//load lexers again, based on the active theme
	void LoadLexers(bool loadDefault);
	void Init(unsigned int uiBuild, const wxChar* version) {
		this->m_svnRevision  = wxString::Format(wxT("%d"), uiBuild);
