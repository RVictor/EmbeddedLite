...
: m_commonConfig(node)
{
	if ( node ) {
		...
		m_buildAsmWithGlobalSettings = XmlUtils::ReadString(node, wxT("BuildAsmWithGlobalSettings"), APPEND_TO_GLOBAL_SETTINGS);
		...
		if (NULL != pAsm) {
			m_assemblerRequired = XmlUtils::ReadBool(compile, wxT("Required"), true);
		}
    ...
    m_assemblerRequired = true;
		...
		m_buildAsmWithGlobalSettings = APPEND_TO_GLOBAL_SETTINGS;
...
wxXmlNode *BuildConfig::ToXml() const
{
	...
	node->AddProperty(wxT("BuildAsmWithGlobalSettings"), m_buildAsmWithGlobalSettings);
	...
	wxXmlNode *pAsm = XmlUtils::FindFirstByTagName(node, wxT("Assembler"));
	if (NULL != pAsm) {
		pAsm->AddProperty(wxT("Required"), BoolToString(m_assemblerRequired));
	}
...
void BuildConfig::SetAsmIncludePath(const wxString &path)
{
	m_commonConfig.SetAsmIncludePath(path);
}
...
wxString BuildConfig::GetAsmIncludePath() const
{
	return m_commonConfig.GetAsmIncludePath();
}
