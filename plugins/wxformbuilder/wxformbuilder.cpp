...
extern "C" EXPORT PluginInfo GetPluginInfo()
{
	...
	info.SetDescription(wxT("wxFormBuilder integration with EmbeddedLite"));
...
{
	m_longName = wxT("wxFormBuilder integration with EmbeddedLite");
...
void wxFormBuilder::OpenWithWxFb(wxCommandEvent& e)
{
	...
	if ( item.m_item.IsOk() && item.m_itemType == ProjectItem::TypeFile ) {
		if (item.m_fileName.GetExt() == wxT("fbp")) {
			...
			wxMessageBox(_("Please select a 'fbp' (Form Builder Project) file only"), wxT("EmbeddedLite"), wxOK|wxCENTER|wxICON_INFORMATION);
			return;
		}
	}
}
