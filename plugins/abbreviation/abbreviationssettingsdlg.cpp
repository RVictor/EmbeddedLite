...
{
	if(m_dirty) {
		if(wxMessageBox(_("Would you like to save the current changes?"), wxT("EmbeddedLite"), wxYES_NO|wxCENTER, this) == wxYES){
			DoSaveCurrent();
		}
	}
...
void AbbreviationsSettingsDlg::OnDelete(wxCommandEvent& event)
{
	...
	if(wxMessageBox(wxString::Format(wxT("Are you sure you want to delete '%s'"), m_activeItemName.c_str()),
			wxT("EmbeddedLite"), wxYES_NO|wxCANCEL|wxCENTER|wxICON_QUESTION, this) != wxYES)
	{
		return;
	}
