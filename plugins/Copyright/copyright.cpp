...
void Copyright::OnInsertCopyrights(wxCommandEvent& e)
{
	...
	if ( !wxFileName::FileExists(data.GetTemplateFilename()) ) {
		wxMessageBox(wxString::Format(_("Template file name '%s', does not exist!"), data.GetTemplateFilename().GetData()), wxT("EmbeddedLite"), wxICON_WARNING|wxOK);
		return;
	}
	...
	if ( !ReadFileWithConversion(data.GetTemplateFilename(), content) ) {
		wxMessageBox(wxString::Format(_("Failed to read template file '%s'"), data.GetTemplateFilename().c_str()), wxT("EmbeddedLite"), wxICON_WARNING|wxOK);
		return;
	}

	IEditor *editor = m_mgr->GetActiveEditor();
	if ( !editor ) {
		wxMessageBox(wxString::Format(_("There is no active editor\n")), wxT("EmbeddedLite"), wxICON_WARNING|wxOK);
		return;
	}
	...
	if ( !l.is_empty() ) {
		if ( wxMessageBox(_("Template file contains text which is not comment, continue anyways?"), wxT("EmbeddedLite"), wxICON_QUESTION|wxYES_NO) == wxNO ) {
			return;
		}
	}
...
void Copyright::OnBatchInsertCopyrights(wxCommandEvent& e)
{
	...
	if (m_mgr->IsWorkspaceOpen() == false) {
		wxMessageBox(_("Batch insert requires a workspace to be opened"), wxT("EmbeddedLite"), wxICON_WARNING|wxOK);
		return;
	}
...
void Copyright::MassUpdate(const std::vector<wxFileName> &filtered_files, const wxString &content)
{
	...
	if (wxMessageBox(wxString::Format(wxT("You are about to modifiy %d files, continue?"), filtered_files.size()), wxT("EmbeddedLite"), wxYES_NO|wxICON_QUESTION) == wxNO) {
		return;
	}
...
bool Copyright::Validate(wxString& content)
{
	...
	// make sure that the template file exists
	if ( !wxFileName::FileExists(data.GetTemplateFilename()) ) {
		wxMessageBox(wxString::Format(wxT("Template file name '%s', does not exist!"), data.GetTemplateFilename().GetData()), wxT("EmbeddedLite"), wxICON_WARNING|wxOK);
		return false;
	}

	// read the copyrights file
	if ( !ReadFileWithConversion(data.GetTemplateFilename(), content) ) {
		wxMessageBox(wxString::Format(wxT("Failed to read template file '%s'"), data.GetTemplateFilename().c_str()), wxT("EmbeddedLite"), wxICON_WARNING|wxOK);
		return false;
	}
