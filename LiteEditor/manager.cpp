...
//---------------------------------------------------------------
//
// The EmbeddedLite manager class
//
//---------------------------------------------------------------
...
{
//rvv	m_codeliteLauncher = wxFileName(wxT("codelite_launcher"));
...
//rvv  if ( Frame::Get()->GetFrameGeneralInfo().GetFlags() & CL_LOAD_LAST_SESSION ) 
	{
...
void Manager::AddToRecentlyOpenedWorkspaces ( const wxString &fileName )
{
	...
	wxString short_name;
	if ( fileName.EndsWith(wxT(".") EL_WORKSPACE_EXT, &short_name) ) {
	...
	for ( size_t i=0; i<files.GetCount(); i++ ) {
		if (!files.Item(i).EndsWith(wxT(".") EL_WORKSPACE_EXT)) {
			files.Item(i).Append(wxT(".") EL_WORKSPACE_EXT);
		}
	}
...
void Manager::DoRestartEmbeddedLite()
{
	wxString command;
#ifdef __WXMSW__
/*//rvv
  // the codelite_launcher application is located where the embeddedlite executable is
	// to properly shoutdown embeddedlite. We first need to close the codelite_indexer process
...
void Manager::SetEmbeddedLiteLauncherPath(const wxString& path)
{
//rvv	m_codeliteLauncher = wxFileName(path, wxT("codelite_launcher"));
}

void Manager::OnRestart(wxCommandEvent& event)
{
	wxUnusedVar(event);
	DoRestartEmbeddedLite();
}
