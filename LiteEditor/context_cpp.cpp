...
void ContextCpp::SwapFiles(const wxFileName &fileName)
{
	...
	if (!userAnsweredBefore) {
		...
		ThreeButtonDlg dlg(Frame::Get(), _("No matched file was found, would you like to create one?"), wxT("EmbeddedLite"));
		res = dlg.ShowModal();
...
void ContextCpp::OnGenerateSettersGetters(wxCommandEvent &event)
{
	...
	if (tag->GetFile().CmpNoCase(editor.GetFileName().GetFullPath()) != 0) {
		...
		msg << wxT("Would you like EmbeddedLite to open this file for you?");

		if (wxMessageBox(msg, wxT("EmbeddedLite"), wxYES_NO) == wxYES) {
...
void ContextCpp::OnOverrideParentVritualFunctions(wxCommandEvent& e)
{
	...
	if (scopeName.IsEmpty() || scopeName == wxT("<global>")) {
		wxMessageBox(_("Cant resolve scope properly. Found <") + scopeName + wxT(">"), wxT("EmbeddedLite"), wxICON_INFORMATION|wxOK);
		return;
	}
...
void ContextCpp::OnAddMultiImpl(wxCommandEvent &e)
{
	...
	wxString scopeName = TagsManagerST::Get()->GetScopeName(context);
	if (scopeName.IsEmpty() || scopeName == wxT("<global>")) {
		wxMessageBox(_("'Add Functions Implementation' can only work inside valid scope, got (") + scopeName + wxT(")"), wxT("EmbeddedLite"), wxICON_INFORMATION|wxOK);
		return;
	}
...
void ContextCpp::OnAddImpl(wxCommandEvent &e)
{
	...
	if (match) {
		...
		if (DoGetFunctionBody(curPos, blockStartPos, blockEndPos, content)) {
			...
			wxMessageBox(_("Function '") + tag->GetName() + wxT("' already has a body"), wxT("EmbeddedLite"), wxICON_WARNING|wxOK);
			return;
		}
