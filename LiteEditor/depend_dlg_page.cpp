...
void DependenciesPage::OnConfigChanged( wxCommandEvent& event )
{
	if(m_dirty) {
		...
		if(wxMessageBox(wxString::Format(wxT("Build order for configuration '%s' has been modified, would you like to save it?"), m_currentSelection.GetData()), wxT("EmbeddedLite"), wxYES_NO|wxICON_QUESTION) == wxYES){
			Save();
		}
		m_dirty = false;
	}
...
{
	wxString errMsg;
	ProjectPtr proj = SolutionST::Get()->FindProjectByName(m_projectName, errMsg);
	if (proj) {
		...
		BuildConfigPtr selBuildConf = SolutionST::Get()->GetProjBuildConf(m_projectName, wxEmptyString);
		if (selBuildConf) {
		...
		wxMessageBox(errMsg, wxT("EmbeddedLite"));
		return;
	}
}
