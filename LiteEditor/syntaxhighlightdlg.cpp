...
wxPanel *SyntaxHighlightDlg::CreateSyntaxHighlightPage()
{
	...
	wxString path = ManagerST::Get()->GetLexersDir() + wxT("/");
//rvv	path << wxT("/lexers/");
...
{
	if(wxMessageBox(_("Are you sure you want to load all default syntax highlight settings and lose all your changes?"), wxT("EmbeddedLite"), wxYES_NO|wxCANCEL|wxICON_QUESTION|wxCENTER, this) != wxYES){
		return;
	}
