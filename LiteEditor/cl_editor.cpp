...
//rvv
static bool
ReadFile8BitData(const char *psczFileName, wxString &psczContent)
{
  wxASSERT(NULL != psczFileName);

	psczContent.Empty();

	FILE *fp = fopen(psczFileName, "rb");
	if (NULL != fp) 
  {
		struct stat buff;
		if ( stat(psczFileName, &buff) == 0 ) 
    {
      ...
      {
				buffer[size] = 0;
				psczContent = wxString::From8BitData(buffer);
			}
			delete [] buffer;
		}
...
bool LEditor::SaveToFile(const wxFileName &fileName)
{
	...
	if (dwAttrs != INVALID_FILE_ATTRIBUTES) {
		if (dwAttrs & FILE_ATTRIBUTE_READONLY) {
			if (wxMessageBox(wxString::Format(wxT("'%s' has read-only attribute set\nWould you like EmbeddedLite to try and remove it?"), fileName.GetFullPath().c_str()), wxT("EmbeddedLite"),
				...
				if ( SetFileAttributes(fileName.GetFullPath().c_str(), dwAttrs & ~(FILE_ATTRIBUTE_READONLY)) == FALSE ) {
					wxMessageBox(wxString::Format(wxT("Failed to open file '%s' for write"), fileName.GetFullPath().c_str()), wxT("EmbeddedLite"), wxOK|wxCENTER|wxICON_WARNING);
					return false;
				}
			} else {
				return false;
  ...
  wxString wxText = GetText();
  char* pszCharBuf = (char*)malloc(wxText.Length()+1);

  if (NULL == pszCharBuf)
  {
		wxMessageBox(wxString::Format(wxT("Cannot save open file '%s' due to encoding conversion problem"), 
      fileName.GetFullPath().GetData()), wxT("EmbeddedLite"));
    return false;
  }
  
  const wxChar* psczEditorData = wxText.GetData();
  size_t i;
  for (i=0; i<wxText.Length(); i++)
  {
    pszCharBuf[i] = (char)(psczEditorData[i]);    //dirty trick!
  }
  pszCharBuf[i] = '\0';
  ...
  {
		// Nothing to be done
		if (wxMessageBox(wxString::Format(wxT("Failed to open file '%s' for write, Override it?"), fileName.GetFullPath().GetData()), wxT("EmbeddedLite"), wxYES_NO|wxICON_WARNING) == wxYES) {
			// try to override it
			...
			if (file.Open(tmp_file.c_str(), wxT("wb")) == false) {
				wxMessageBox(wxString::Format(wxT("Failed to open file '%s' for write"), tmp_file.c_str()), wxT("EmbeddedLite"), wxOK|wxICON_WARNING);
        free(pszCharBuf);
				return false;
			}
		}
...
*/  
	file.Write(pszCharBuf, strlen(pszCharBuf));
	file.Close();
  free(pszCharBuf);
	...
	if (tmp_file.IsEmpty() == false) {
		if (wxRenameFile(tmp_file, fileName.GetFullPath(), true) == false) {
			wxMessageBox(wxString::Format(wxT("Failed to override read-only file")), wxT("EmbeddedLite"), wxOK|wxICON_WARNING);
			return false;
		}
	}
...
//----------------------------------------------------------------
// Demonstrate how to achieve symbol browsing using the EmbeddedLite
...
void LEditor::OnFindDialog(wxCommandEvent& event)
{
	...
	if ( type == wxEVT_FRD_FIND_NEXT ) {
		...
		if ( !Replace() ) {
			...
			wxString msg;
			if ( dirDown ) {
				msg << wxT("EmbeddedLite reached the end of the document, Search again from the start?");
			} else {
				msg << wxT("EmbeddedLite reached the end of the document, Search again from the bottom?");
			}

			if (!conf->GetLongValue(wxT("ReplaceWrapAroundAnswer"), res)) {
				ThreeButtonDlg *dlg = new ThreeButtonDlg(NULL, msg, wxT("EmbeddedLite"));
				res = dlg->ShowModal();
...
void LEditor::FindNext(const FindReplaceData &data)
{
	...
	if ( !FindAndSelect(data) ) {
		...
		wxString msg;
		if ( dirDown ) {
			msg << _("EmbeddedLite reached the end of the document, Search again from the start?");
		} else {
			msg << _("EmbeddedLite reached the top of the document, Search again from the bottom?");
		}
...
void LEditor::ReloadFile()
{
	...
	wxString text;
//rvv	ReadFileWithConversion(m_fileName.GetFullPath(), text);

  // rvv {
  // try local 8 bit data
  const wxCharBuffer name = _C(m_fileName.GetFullPath());
  ReadFile8BitData(name.data(), text);
  // rvv }
