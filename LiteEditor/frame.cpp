...
// from iconsextra.cpp:
extern char *gl_chXpmChipC_16x16[];
//nn extern unsigned char cubes_alpha[];
...
BEGIN_EVENT_TABLE(Frame, wxFrame)
	...
	// Project menu
	//-------------------------------------------------------
	EVT_MENU(XRCID("local_solution_prefs"),    Frame::OnWorkspaceEditorPreferences)
	EVT_MENU(XRCID("local_solution_settings"), Frame::OnWorkspaceSettings)
	EVT_MENU(XRCID("new_solution"),            Frame::OnProjectNewWorkspace)
	EVT_MENU(XRCID("open_solution"),      Frame::OnSwitchWorkspace)
	EVT_MENU(XRCID("close_solution"),          Frame::OnCloseWorkspace)
	EVT_MENU(XRCID("reload_solution"),         Frame::OnReloadWorkspace)
	...
	EVT_MENU(XRCID("retag_solution"),          Frame::OnRetagWorkspace)
	EVT_MENU(XRCID("full_retag_solution"),     Frame::OnRetagWorkspace)
	EVT_MENU(XRCID("project_properties"),       Frame::OnShowActiveProjectSettings)

	EVT_UPDATE_UI(XRCID("local_solution_prefs"),    Frame::OnWorkspaceOpen)
	EVT_UPDATE_UI(XRCID("local_solution_settings"), Frame::OnWorkspaceOpen)
	EVT_UPDATE_UI(XRCID("close_solution"),          Frame::OnWorkspaceOpen)
	EVT_UPDATE_UI(XRCID("reload_solution"),         Frame::OnReloadWorkspaceUI)
	EVT_UPDATE_UI(XRCID("add_project"),              Frame::OnWorkspaceMenuUI)
	EVT_UPDATE_UI(XRCID("retag_solution"),          Frame::OnWorkspaceOpen)
	EVT_UPDATE_UI(XRCID("full_retag_solution"),     Frame::OnWorkspaceOpen)
	...
	EVT_MENU(XRCID("build_solution"),          Frame::OnBuildWorkspace)
	EVT_MENU(XRCID("clean_solution"),          Frame::OnCleanWorkspace)
	EVT_MENU(XRCID("rebuild_solution"),        Frame::OnReBuildWorkspace)
	...
	EVT_UPDATE_UI(XRCID("build_solution"),         Frame::OnBuildWorkspaceUI)
	EVT_UPDATE_UI(XRCID("clean_solution"),         Frame::OnCleanWorkspaceUI)
	EVT_UPDATE_UI(XRCID("rebuild_solution"),       Frame::OnReBuildWorkspaceUI)
	...
	// Plugins menu
	//-------------------------------------------------------
//rvv	EVT_MENU(XRCID("manage_plugins"),           Frame::OnManagePlugins)
...
//rvv	EVT_MENU(XRCID("check_for_update"),         Frame::OnCheckForUpdate)
	...
	//-------------------------------------------------------
	// Solution Pane tab context menu
	//-------------------------------------------------------
	...
	EVT_MENU   (XRCID("goto_codelite_download_url"),               Frame::OnGotoEmbeddedLiteDownloadPage)
...
//nn	JobQueueSingleton::Instance()->PushJob(new SingleInstanceThreadJob(this, ManagerST::Get()->GetStarupDirectory()));
	JobQueueSingleton::Instance()->PushJob(new SingleInstanceThreadJob(this, ManagerST::Get()->GetAppShareDir()));    //rvv,todo
...
void
Frame::Initialize(bool loadLastSession)
{
	//set the revision number in the frame title
	wxString title(wxT("EmbeddedLite - Revision: "));
	title << APP_VER_BUILD;
	...
	{
		frameSize.SetHeight(600);
	}

	inf.SetFrameSize( frameSize );

	m_theFrame = new Frame( NULL, wxID_ANY, title, inf.GetFramePosition(), inf.GetFrameSize(), 
	...
	m_theFrame->GetFileSystemBrowser()->GetEventHandler()->ProcessEvent(e);
...
void
Frame::CreateGUIControls(void)
{
#ifdef __WXMSW__
	SetIcon(wxICON(aaaaa));
#else
	wxImage img(gl_chXpmChipC_16x16);
//nn	img.SetAlpha(cubes_alpha, true);
	wxBitmap bmp(img);
	wxIcon icon;
...
// Mac / Linux
	...
	DebuggerConfigTool::Get()->Load(ManagerST::Get()->GetConfigDir() + wxT("/debuggers.xml"), wxT("2.0.2"));
	SolutionST::Get()->SetStartupDir(ManagerST::Get()->GetStarupDirectory());
	...
	m_workspacePane = new CSolutionPanel(m_mainPanel, wxT("Solution View"), &m_mgr);
	...
	RegisterDockWindow(XRCID("workspace_pane"), wxT("Solution View"));
	...
	BuildSettingsConfigST::Get()->Load(wxT("2.0.4"), ManagerST::Get()->GetConfigDir());
	...
	tagsManager->SetEmbeddedLiteIndexerPath(wxStandardPaths::Get().GetDataDir());
#else
	// set the path to codelite_indexer
	wxFileName exePath( wxStandardPaths::Get().GetExecutablePath() );
	tagsManager->SetEmbeddedLiteIndexerPath(exePath.GetPath());
	ManagerST::Get()->SetEmbeddedLiteLauncherPath(exePath.GetPath());
#endif
	...
	//load debuggers
	DebuggerMgr::Get().Initialize(this, EnvironmentConfig::Instance(), ManagerST::Get()->GetAppDataDir());
	DebuggerMgr::Get().LoadDebuggers(ManagerST::Get()->GetAppShareDir());

	wxString sessConfFile;
	sessConfFile << ManagerST::Get()->GetConfigDir() << wxT("/sessions.xml");
...
wxString
Frame::GetViewAsLanguageById(int id) const
{
...
void Frame::CreateToolbars24()
{
	...
	tb->AddSeparator();
	tb->AddTool(XRCID("find_resource"), wxT("Find Resource In Solution"), wxXmlResource::Get()->LoadBitmap(wxT("open_resource24")), wxT("Find Resource In Workspace"));
	tb->AddTool(XRCID("find_type"), wxT("Find Type In Solution"), wxXmlResource::Get()->LoadBitmap(wxT("open_type24")), wxT("Find Type In Workspace"));
...
void Frame::CreateToolbars16()
{
	...
	tb->AddTool(XRCID("find_resource"), wxT("Find Resource In Solution"), wxXmlResource::Get()->LoadBitmap(wxT("open_resource16")), wxT("Find Resource In Workspace"));
	tb->AddTool(XRCID("find_type"), wxT("Find Type In Solution"), wxXmlResource::Get()->LoadBitmap(wxT("open_type16")), wxT("Find Type In Workspace"));
...
//rvv	const wxString ALL(wxT("EmbeddedLite Solution files (*.workspace)|*.workspace|")
//rvv	                   wxT("All Files (*)|*"));
	wxString ALL = wxString::Format(wxT("EmbeddedLite Solution files (*.%s)|*.%s|"), EL_WORKSPACE_EXT, EL_WORKSPACE_EXT) + 
	                   wxT("All Files (*)|*");
...
void Frame::OnFileSaveTabGroup(wxCommandEvent& WXUNUSED(event))
{
	...
	while (true) {
		...
		if (sessionName.IsEmpty()) {
			if ( wxMessageBox(_("Please enter a name for the tab group"), wxT("EmbeddedLite"), wxICON_ERROR|wxOK|wxCANCEL, this) != wxOK ) {
				return;
			} else {
				continue;
			}
		...
		if (path.IsEmpty() || !wxFileName::DirExists(path)) {
			if ( wxMessageBox(_("Please enter a valid directory in which to save the tab group"), wxT("EmbeddedLite"), wxICON_ERROR|wxOK|wxCANCEL, this) != wxOK ) {
				return;
			} else {
				continue;
			}
...
//rvv	const wxString ALL(	wxT("EmbeddedLite Projects (*.project)|*.project|")
//rvv	                    wxT("All Files (*)|*"));
	wxString ALL = wxString::Format(wxT("EmbeddedLite Projects (*.%s)|*.%s|"), EL_PROJECT_EXT, EL_PROJECT_EXT) + 
	                    wxT("All Files (*)|*");
...
void Frame::OnBuildCustomTarget(wxCommandEvent& event)
{
	...
	if (enable) {
		...
		TreeItemInfo item = GetSolutionTab()->GetFileView()->GetSelectedItemInfo();
...
void
Frame::OnTimer(wxTimerEvent &event)
{
	...
	if ( check ) {
//rvv		JobQueueSingleton::Instance()->PushJob(new WebUpdateJob(this, false));
	}
	...
	if (cpus != wxNOT_FOUND) 
	{
		...
		if ( bs ) 
		{
			...
			if ( bs->GetToolJobs() != jobs ) 
			{
				...
				GetMainBook()->ShowMessage(wxT("Should EmbeddedLite adjust the number of concurrent build jobs to match the number of CPUs?"), true, wxXmlResource::Get()->LoadBitmap(wxT("message_pane_fix")), btn1);
			}
		}
		...
		if ( m_tagsOptionsData.GetParserSearchPaths().IsEmpty() ) {
			...
			locator.Locate( paths, excudePaths, ManagerST::Get()->GetConfigDir() );

			if ( paths.IsEmpty() && updatePaths) {
				GetMainBook()->ShowMessage(
								wxT("EmbeddedLite could not find any search paths set for the code completion parser\n")
								wxT("This means that EmbeddedLite will *NOT* be able to offer any code completion for non-workspace files (e.g. string.h).\n")
				...
				if(updatePaths) {
					...
					GetMainBook()->ShowMessage(
									wxT("Should EmbeddedLite update your code completion parser search paths ? (since there are none..)"),
...
wxString Frame::CreateWorkspaceTable()
{
	...
	if (files.GetCount() == 0) {
		...
		html << wxT("No solutions found.") << wxT("</font></td></tr>");
	} else {
...
void Frame::CreateRecentlyOpenedWorkspacesMenu()
{
	...
	if (idx != wxNOT_FOUND) {
		wxMenu *menu = GetMenuBar()->GetMenu(idx);
		wxMenu *submenu = NULL;
		wxMenuItem *item = menu->FindItem(XRCID("recent_solutions"));
		if (item) {
			submenu = item->GetSubMenu();
		}
...
void Frame::OnRecentWorkspace(wxCommandEvent &event)
{
	...
	if (idx < files.GetCount()) {
		...
		if (!file_name.EndsWith(wxT(".") EL_WORKSPACE_EXT))
    {
...
void Frame::CreateWelcomePage()
{
	Manager *mgr = ManagerST::Get();
	//load the template
	wxFileName fn(mgr->GetAppShareDir(), wxT("index.html"));      //todo "startup" vs GetAppShareDir
	...
	//replace $(InstallPath)
	content.Replace(wxT("$(InstallPath)"), mgr->GetAppShareDir());    //todo: "startup" vs GetAppShareDir
...
void Frame::OnDebug(wxCommandEvent &e)
{
	...
	if (dbgr && dbgr->IsRunning()) {
		...
		if (SolutionST::Get()->GetActiveProjectName().IsEmpty()) {
		...
		if (!EditorConfigST::Get()->GetLongValue(wxT("BuildBeforeDebug"), build_first)) {
			...
			ThreeButtonDlg *dlg = new ThreeButtonDlg(this, wxT("Would you like to build the project before debugging it?"), wxT("EmbeddedLite"));
		...
		if (build_first == wxID_OK) {
			QueueCommand bldCmd(SolutionST::Get()->GetActiveProjectName(), wxEmptyString, false, QueueCommand::Build);

			// handle custom builds
			BuildConfigPtr bldConf = SolutionST::Get()->GetProjBuildConf(SolutionST::Get()->GetActiveProjectName(), wxEmptyString);
...
void Frame::OnLinkClicked(wxHtmlLinkEvent &e)
{
	...
	if (command != wxT("switch-workspace") &&
	        ...
	        command != wxT("create-solution") &&
	        command != wxT("import-msvs-solution") &&
	        command != wxT("open-solution")) {
		e.Skip();
		return;
	}
	...
	if (wxFileName(filename).GetExt() == EL_WORKSPACE_EXT) 
  {
...
void Frame::OnStartPageEvent(wxCommandEvent& e)
{
	...
	if ( data->action == wxT("switch-workspace")) {
		...
		OnImportMSVS(e);
	} else if ( data->action == wxT("open-solution")) {
		OnSwitchWorkspace(e);
	}
	delete data;
}
...
CSolutionTab *Frame::GetSolutionTab()
{
	return GetWorkspacePane()->GetSolutionTab();
}

CFileSystemBrowser *Frame::GetFileSystemBrowser()
{
	return GetWorkspacePane()->GetFileSystemBrowser();
}
...
void Frame::OnSingleInstanceOpenFiles(wxCommandEvent& e)
{
	...
	if (arr) {
			...
			if (fn.GetExt() == EL_WORKSPACE_EXT) 
      {
				if ( ManagerST::Get()->IsWorkspaceOpen() ) 
        {
					if (wxMessageBox(_("Close this workspace, and load workspace '") + fn.GetFullName() + wxT("'"), wxT("EmbeddedLite"), wxICON_QUESTION|wxYES_NO, this) == wxNO) 
          {
						continue;
					}
				}
...
void Frame::OnNewVersionAvailable(wxCommandEvent& e)
{
	...
	if (data) {
		if (data->IsUpToDate() == false) {
			...
			GetMainBook()->ShowMessage(wxT("A new version of EmbeddedLite is available. Download it?"), true, wxXmlResource::Get()->LoadBitmap(wxT("message_pane_software_update")), btn);

		} else {
			if(!data->GetShowMessage()) {
				wxLogMessage(wxString::Format(wxT("Info: EmbeddedLite is up-to-date (or newer), version used: %d, version on site:%d"), data->GetCurrentVersion(), data->GetNewVersion()));

			} else {
				// User initiated the version check request
				GetMainBook()->ShowMessage(wxT("EmbeddedLite is up-to-date"));
			}
		}
		delete data;
	}
...
void Frame::SetFrameTitle(LEditor* editor)
{
	...
	title << wxT("EmbeddedLite - Revision: ");
	title << APP_VER_BUILD;
	SetTitle(title);
}
...
{
	wxUnusedVar(event);
	ManagerST::Get()->RetagWorkspace(event.GetId() == XRCID("retag_solution") ? true : false );
}
...
void Frame::ReloadExternallyModifiedProjectFiles()
{
	...
	CSolution* workspace = SolutionST::Get();
	...
	btn.buttonLabel = wxT("Reload Solution");
	btn.commandId   = XRCID("reload_solution");
	btn.isDefault   = false;
	btn.window      = this;
	...
	GetMainBook()->ShowMessage(_("Solution or project settings have been modified, would you like to reload the workspace and all contained projects?"), false, wxXmlResource::Get()->LoadBitmap(wxT("message_pane_reload_workspace")), noBtn, btn);
}

void Frame::SaveLayoutAndSession()
{
	//save the perspective
	WriteFileUTF8(ManagerST::Get()->GetConfigDir() + wxT("/embeddedlite.layout"), m_mgr.SavePerspective());
	...
	EditorConfigST::Get()->SaveLongValue(wxT("SolutionView"), GetWorkspacePane()->GetNotebook()->GetBookStyle());
...
{
	ButtonDetails btn;
	btn.buttonLabel = wxT("Retag Solution Now!");
	btn.commandId   = XRCID("full_retag_solution");
	btn.window      = this;

	GetMainBook()->ShowMessage(wxT("Your workspace symbols file does not match the current version of EmbeddedLite. This can be fixed by re-tagging your workspace\nWould you like to re-tag your workspace now?"), true, wxNullBitmap, btn);
}
...
{
//rvv	JobQueueSingleton::Instance()->PushJob( new WebUpdateJob(this, true) );
}

void Frame::OnShowActiveProjectSettings(wxCommandEvent& e)
{
	GetSolutionTab()->GetEventHandler()->ProcessEvent(e);
}
...
void Frame::OnShowActiveProjectSettingsUI(wxUpdateUIEvent& e)
{
	...
	SolutionST::Get()->GetProjectList( projectList );
...
void Frame::OnLoadPerspective(wxCommandEvent& e)
{
	...
	if (loadIt) {
		...
		wxString file_name(ManagerST::Get()->GetConfigDir() + wxT("/embeddedlite.layout"));
		...
		if ( !pers.IsEmpty() && (EditorConfigST::Get()->GetRevision() == wxString::Format(wxT("%d"), APP_VER_BUILD)) ) 
		{
			m_mgr.LoadPerspective(pers);
		}
		else
		{
			EditorConfigST::Get()->SetRevision(wxString::Format(wxT("%d"), APP_VER_BUILD));
		}
	}
	...
	// unless they delete their embeddedlite.layout file, which people usually
	// dont do
...
{
	GetSolutionTab()->GetFileView()->GetEventHandler()->ProcessEvent(e);
}
...
void Frame::OnGotoEmbeddedLiteDownloadPage(wxCommandEvent& e)
{
	wxUnusedVar(e);
...
{
#ifdef __WXMAC__
	GetMainBook()->ShowMessage(_("Some of the changes made requires restart of EmbeddedLite"));
#else
	...
	GetMainBook()->ShowMessage(_("Some of the changes made requires a restart of EmbeddedLite, Restart now?"), false, wxXmlResource::Get()->LoadBitmap(wxT("message_pane_restart")), btn1, btn2);
#endif
}
