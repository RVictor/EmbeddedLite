		...
		, m_selecteCustomTaregt(wxNOT_FOUND)
{
	...
	m_choiceAsmUseWithGlobalSettings->AppendString(APPEND_TO_GLOBAL_SETTINGS);
	m_choiceAsmUseWithGlobalSettings->AppendString(OVERWRITE_GLOBAL_SETTINGS);
	m_choiceAsmUseWithGlobalSettings->AppendString(PREPEND_GLOBAL_SETTINGS);
	m_choiceAsmUseWithGlobalSettings->Select(0);
	...
	if (m_checkEnableCustomBuild->IsChecked()) {
		...
		DisableAssemblerPage(true);
		DisableCompilerPage(true);
		m_checkCompilerNeeded->Enable(false);
		m_pCntlChkAssemblerNeeded->Enable(false);
		...
		if (m_pCntlChkAssemblerNeeded->IsChecked()) {
			DisableAssemblerPage(true);
		}
...
void ProjectConfigurationPanel::ClearValues()
{
  ...
  m_textAssemblerOptions->SetValue(wxEmptyString);
  m_pCntlChkAssemblerNeeded->SetValue(true);
	DisableAssemblerPage(m_pCntlChkAssemblerNeeded->IsChecked());
...
void ProjectConfigurationPanel::CopyValues(const wxString &confName)
{
	...
	m_pCntlChkAssemblerNeeded->SetValue(!buildConf->IsAssemblerRequired());
	m_textAssemblerOptions->SetValue(buildConf->GetAssemblerOptions());
  DisableAssemblerPage(m_pCntlChkAssemblerNeeded->IsChecked());
	m_txtAsmAdditionalSearchPath->SetValue(buildConf->GetAsmIncludePath());
	...
	SelectChoiceWithGlobalSettings(m_choiceAsmUseWithGlobalSettings, buildConf->GetBuildAsmWithGlobalSettings());
...
void ProjectConfigurationPanel::SaveValues(const wxString &confName)
{
	...
	buildConf->SetAssemblerOptions(m_textAssemblerOptions->GetValue());
	buildConf->SetAsmIncludePath(m_txtAsmAdditionalSearchPath->GetValue());
	...
	useWithGlobalSettings = m_choiceAsmUseWithGlobalSettings->GetStringSelection();
	if (useWithGlobalSettings == APPEND_TO_GLOBAL_SETTINGS) {
		buildConf->SetBuildAsmWithGlobalSettings(BuildConfig::APPEND_TO_GLOBAL_SETTINGS);
	} else if (useWithGlobalSettings == OVERWRITE_GLOBAL_SETTINGS) {
		buildConf->SetBuildAsmWithGlobalSettings(BuildConfig::OVERWRITE_GLOBAL_SETTINGS);
	} else if (useWithGlobalSettings == PREPEND_GLOBAL_SETTINGS) {
		buildConf->SetBuildAsmWithGlobalSettings(BuildConfig::PREPEND_GLOBAL_SETTINGS);
	}  
...
void ProjectConfigurationPanel::DoUpdatePages(bool checked)
{
	...
	if (checked) {
		...
		m_pCntlChkAssemblerNeeded->Enable(true);
...
void ProjectConfigurationPanel::DisableAssemblerPage(bool disable)
{
	m_staticText3312->Enable(!disable);
	m_choiceAsmUseWithGlobalSettings->Enable(!disable);
	m_staticText61->Enable(!disable);
	m_staticText41->Enable(!disable);
	m_textAssemblerOptions->Enable(!disable);
	m_buttonAssemblerOptions->Enable(!disable);
	m_staticText1711->Enable(!disable);
	m_txtAsmAdditionalSearchPath->Enable(!disable);
	m_pCntlBtnAsmAddSearchPath->Enable(!disable);
}
...
void ProjectConfigurationPanel::OnCheckAssemblerNeeded(wxCommandEvent &event)
{
	DisableAssemblerPage(event.IsChecked());
	SetSettingsModified();
	event.Skip();
}
...
void
ProjectConfigurationPanel::OnAddAsmSearchPath(wxCommandEvent &event)
{
	if (PopupAddOptionDlg(m_txtAsmAdditionalSearchPath)) 
  {
		SetSettingsModified();
	}
	event.Skip();
...
void
ProjectConfigurationPanel::OnButtonAddAssemblerOptions(wxCommandEvent &event)
{
	wxString cmpName = m_choiceCompilerType->GetStringSelection();
	CompilerPtr cmp = BuildSettingsConfigST::Get()->GetCompiler(cmpName);
  
	if (PopupAddOptionCheckDlg(m_textAssemblerOptions, _("Assembler options"), cmp->GetAssemblerOptions())) 
  {
		SetSettingsModified();
	}
	event.Skip();
...
void
ProjectConfigurationPanel::OnBtnAddAsmPreprocessor(wxCommandEvent &event)
{
	if (PopupAddOptionDlg(m_txtAsmPreprocessor)) 
  {
		SetSettingsModified();
	}
...
void ProjectConfigurationPanel::OnNewTarget(wxCommandEvent& e)
{
	...
	if (dlg.ShowModal() == wxID_OK) {
		...
		if (GetTargetCommand(dlg.GetName()).IsEmpty() == false) {
			wxMessageBox(wxString::Format(wxT("Target '%s' already exist!"), dlg.GetName().c_str()), wxT("EmbeddedLite"), wxICON_WARNING|wxCENTER|wxOK, this);
			return;
		}
		long item = AppendListCtrlRow(m_listCtrlTargets);
...
void
ProjectConfigurationPanel::OnBrowseBuildDir(wxCommandEvent& e)
{
	DirSaver ds;
	...
	if (p) 
  {
	...
	wxFileName fn(m_txtBuildDirectory->GetValue());
	wxString initPath(wxEmptyString);
	...
	if (fn.DirExists()) 
  {
	...
	wxString new_path = wxDirSelector(wxT("Select build directory:"), initPath, wxDD_DEFAULT_STYLE, wxDefaultPosition, this);
	if (!new_path.IsEmpty()) 
  {
		m_txtBuildDirectory->SetValue(new_path);
	}
}
...
void GlobalSettingsPanel::ClearValues()
{
	...
	m_textAssemblerOptions->SetValue(wxEmptyString);
  m_txtAsmAdditionalSearchPath->SetValue(wxEmptyString);
	m_txtAsmPreprocessor->SetValue(wxEmptyString);
...
void GlobalSettingsPanel::CopyValues()
{
	...
	m_textAssemblerOptions->SetValue(globalSettings->GetAssemblerOptions());
  m_txtAsmAdditionalSearchPath->SetValue(globalSettings->GetAsmIncludePath());
//todo  m_txtAsmPreprocessor->SetValue(globalSettings->GetAsmPreprocessor());
...
void GlobalSettingsPanel::SaveValues()
{
	...
	globalSettings->SetAssemblerOptions(m_textAssemblerOptions->GetValue());
	globalSettings->SetAsmIncludePath(m_txtAsmAdditionalSearchPath->GetValue());
//todo	globalSettings->SetAsmPreprocessor(m_txtAsmPreprocessor->GetValue());
...
void GlobalSettingsPanel::OnAddAsmSearchPath(wxCommandEvent &event)
{
	if (PopupAddOptionDlg(m_txtAsmAdditionalSearchPath)) {
		SetSettingsModified();
	}
...
void 
GlobalSettingsPanel::OnButtonAddAssemblerOptions(wxCommandEvent &event)
{
	// This is not perfect : I just take the first compiler to find options
	BuildSettingsConfigCookie cookie;
	CompilerPtr cmp = BuildSettingsConfigST::Get()->GetFirstCompiler(cookie);
...
void
GlobalSettingsPanel::OnBtnAddAsmPreprocessor(wxCommandEvent &event)
{
	if (PopupAddOptionDlg(m_txtAsmPreprocessor)) 
  {
		SetSettingsModified();
