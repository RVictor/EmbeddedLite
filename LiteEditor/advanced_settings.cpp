...
#define EMBL_COMPILER_ROOT                "Compiler"
#define EMBL_COMPILER_COMPILER_ELEM       "CompilerSettings"
#define EMBL_COMPILER_LEXERS_ELEM         "Lexers"
#define EMBL_COMPILER_LEXER_ELEM          "Lexer"
		...
		, m_rightclickMenu(NULL)
{
	...
	m_buttonImportCompiler = new wxButton( m_compilersPage, wxID_ANY, wxT("Import"), wxDefaultPosition, wxDefaultSize, 0 );
	bSizer4->Add( m_buttonImportCompiler, 0, wxALL|wxALIGN_RIGHT, 5 );

	m_buttonExportCompiler = new wxButton( m_compilersPage, wxID_ANY, wxT("Export"), wxDefaultPosition, wxDefaultSize, 0 );
	bSizer4->Add( m_buttonExportCompiler, 0, wxALL|wxALIGN_RIGHT, 5 );
	...
	ConnectButton(m_buttonImportCompiler, AdvancedDlg::OnButtonImportClicked);
	ConnectButton(m_buttonExportCompiler, AdvancedDlg::OnButtonExportClicked);
...
void
AdvancedDlg::OnButtonImportClicked(wxCommandEvent &event)
{
	wxUnusedVar(event);

  wxString strCompilerPath = wxFileSelector(wxT("Select file:"), wxT(""), wxT(""), wxT(".compiler"), 
     wxT("Compiler description files (*.compiler)|*.compiler|All files (*.*)|*.*"), wxFD_OPEN|wxFD_FILE_MUST_EXIST, this);

  if (!strCompilerPath.IsEmpty())
  {
    wxXmlDocument Doc;
    wxXmlNode* pCompilerNode = NULL;

    if ( Doc.Load(strCompilerPath) && (NULL != Doc.GetRoot())
      && (NULL != (pCompilerNode = XmlUtils::FindFirstByTagName(Doc.GetRoot(), wxT(EMBL_COMPILER_COMPILER_ELEM)))) )
    {
      wxTextEntryDialog *dlg = new wxTextEntryDialog(this, wxT("Enter New Compiler Name:"), wxT("New Compiler"), wxT("New Compiler"));
      if (dlg->ShowModal() == wxID_OK)
      {
        wxString strName = dlg->GetValue();
        TrimString(strName);
        if (!strName.IsEmpty()) 
        {
          if (BuildSettingsConfigST::Get()->IsCompilerExist(strName)) 
          {
            wxMessageBox(_("A compiler with this name already exist"), wxT("Error"), wxOK|wxICON_HAND);
          }
          else
          {
            CompilerPtr pCompiler = new Compiler(pCompilerNode);
            pCompiler->SetName(strName);
...
void
AdvancedDlg::OnButtonExportClicked(wxCommandEvent &event)
{
	wxUnusedVar(event);
	...
	if (wxNOT_FOUND != sel) 
  {
		wxString strName = m_compilersNotebook->GetPageText((size_t)sel);
    wxString strCompilerPath = wxT("") + strName + wxT(".compiler");
    strCompilerPath = wxFileSelector(wxT("Select file:"), wxT(""), strCompilerPath, wxT(".compiler"), 
       wxT("Compiler description files (*.compiler)|*.compiler|All files (*.*)|*.*"), wxFD_SAVE|wxFD_OVERWRITE_PROMPT , this);
       
    if (!strCompilerPath.IsEmpty())
    {
      CompilerPtr ptrCompiler = BuildSettingsConfigST::Get()->GetCompiler(strName);
      if (ptrCompiler)
      {
        wxXmlDocument Doc;
        wxXmlNode* pCompilerNode = NULL;

        Doc.SetRoot(new wxXmlNode(NULL, wxXML_ELEMENT_NODE, wxT(EMBL_COMPILER_ROOT)));

        if ( (NULL != Doc.GetRoot()) && (NULL != (pCompilerNode = ptrCompiler->ToXml())) )
        {
          Doc.GetRoot()->SetChildren(pCompilerNode);
          pCompilerNode->SetName(wxT(EMBL_COMPILER_COMPILER_ELEM));
        
          Doc.Save(strCompilerPath);
        }
      }
    }
	}
...
void AdvancedDlg::AddCompiler(CompilerPtr cmp, bool selected)
{
	...
	CompilerAssemblerOptionsPage *p6_1 = new CompilerAssemblerOptionsPage(m_compilersNotebook, cmp->GetName());
	pages.push_back(p6_1);
	m_compilersNotebook->AddSubPage(p6_1, _("Assembler options"), false);
