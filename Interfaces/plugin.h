 ...
 * @date 05/07/08
 * @file plugin.h
 * @brief The interface that defines a plugin for EmbeddedLite. each plugin must implement the pure virtual methods of this
 * interface. The plugin gains access to EmbeddedLite data by using the m_mgr member
 * @sa  IManager
 */
...
public:
    ...
    wxString pluginsDir(m_mgr->GetInstallDirectory());
		wxString basePath(pluginsDir + wxT("/resources/"));     //todo?
