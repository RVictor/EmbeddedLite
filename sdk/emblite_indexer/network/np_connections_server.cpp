...
#ifdef __WXMSW__
static PIPE_HANDLE 
createNamedPipe(const char* pipeName, SECURITY_ATTRIBUTES sa)
{
  wchar_t psczUnicodeFName[1024];
  MultiByteToWideChar(CP_ACP, 0, pipeName, -1, psczUnicodeFName, 1024);	    //todo: countof

	return CreateNamedPipe(psczUnicodeFName,
...
clNamedPipe *clNamedPipeConnectionsServer::waitForNewConnection( int timeout )
{
	...
	bool fConnected = (FALSE != ConnectNamedPipe(hConn, &ov));
	if (fConnected != 0) {
